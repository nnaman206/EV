...
function Login_user() {
  ...
  const confirm = async (e) => {
    ...
    if (!validateEmail(email)) {
      return clipAct("Invalid Email Entered");
    }
    if (!validatePassword(password)) {
      return clipAct("Invalid Password Entered");
    }
    ...
    try {
      ...
      if (response.ok) {
        if (data.user && (data.user.id || data.user._id)) {
          ...
          clipAct(`Welcome back, ${data.user.name || email.split('@')[0]}!`);
          navigate(`/userDashboard/${userId}`); // Navigate with proper user ID
        } else {
          clipAct("User ID not found in response.");
        }
      } else {
        clipAct(data.message || "Login failed");
      }
    } catch (error) {
